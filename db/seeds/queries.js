...
const allUsers = () => {
  return db.query("SELECT DISTINCT username FROM users").then((results) => {
...
const commentsZerVotes = () => {
  return db
    .query("SELECT * FROM comments WHERE votes = $1", [0])
    .then((result) => {
      console.log(result.rows);
    });
};
...
const getAllTopics = () => {
  return db
    .query(
      "SELECT DISTINCT slug FROM topics UNION SELECT DISTINCT topic FROM articles",
    )
    .then((result) => {
      console.log(result.rows);
    });
...
const articlesByUser = () => {
  return db
    .query("SELECT * FROM articles WHERE author = $1", ["grumpy19"])
    .then((result) => {
      console.log(result.rows);
    });
};
